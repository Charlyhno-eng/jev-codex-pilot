...
export function Plan({ analysis, adjustable, onAdjust }: { analysis: Analysis; adjustable: boolean; onAdjust: (dimension: "model" | "reasoning", delta: -1 | 1) => Promise<void> }) {
  ...
  const modelIndex = modelLevels.indexOf(analysis.model);
  const reasoningIndex = reasoningLevels.indexOf(analysis.reasoning);
  const adjustment = (dimension: "model" | "reasoning", delta: -1 | 1) => void onAdjust(dimension, delta);
  const modelName = models[analysis.model] ?? analysis.model;
  return <div className="plan-grid"><div className="plan-summary">
    ...
    {analysis.precision_score !== undefined && analysis.precision_score < 70 && <p className="precision-advisory">This task is still valid, but JEV recommends adding more detail before Codex starts.</p>}
  ...
  </div><div className="scope-panel"><p className="panel-label">MINIMAL CONTEXT</p><FileGroup title="Context" values={analysis.context_files}/><FileGroup title="Likely to modify" values={analysis.files_to_modify}/></div></div>;
}
