# Archived AGENTS.md notes

This snapshot preserves earlier delivery notes. Current instructions are in the root `AGENTS.md`.
...
- Added orange backend logs for each JEV decision offload, shell gate, and context-diet hook action.
...
- Paused tickets on Codex credit exhaustion at any turn, scheduled resumption one minute after the reported reset, and logged session transitions in violet.
